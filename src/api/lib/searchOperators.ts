...
const replaceAsterisksAndQuotes = (str: any) => {
  let res = str;
...
export default {
  ...
  firstLetterStartToQuery: (charCodeQuery: any, charCodeQueryWildcard: any) => {
  ...
  firstLetterAnywhereToQuery: (charCodeQuery: any, charCodeQueryWildcard: any) => {
    if (constantsObj.SearchOperators.some(operator => charCodeQuery.includes(operator))) {
      // eslint-disable-next-line no-control-regex
      const seperateAtPlusorMinus = /[+-]?[\x00-\x2A\x2C\x2A\x2E-\x7F]+/g;
      const matches = charCodeQuery.match(seperateAtPlusorMinus);
  ...
  fullWordRomanizedToQuery: (searchQuery: any) => {
    if (constantsObj.SearchOperators.some(operator => searchQuery.includes(operator))) {
      ...
      matches.forEach((match: any) => {
        if (match.includes('+') || (!match.includes('+') && !match.includes('-'))) {
          ...
          spicyWords = spicyWords.map((word: any) => word.substr(0, 1));
    ...
    spicy = spicy.map((word: any) => word.substr(0, 1));
  ...
  fullWordGurmukhiToQuery: (searchQuery: any) => {
  ...
  fullWordEnglishToQuery: (searchQuery: any) => {
